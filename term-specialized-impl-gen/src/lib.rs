...
use langspec::{
    ...
    langspec::{AlgebraicSortId, LangSpec, Name, TerminalLangSpec as _},
...
pub fn generate<L: LangSpec>(
    ...
    let owned_mod = gen_owned_mod(bps, data_structure_lsg, term_trait_lsg);
...
pub(crate) fn gen_ccf_impls<L: LangSpec>(bps: &BasePaths, tgd: &TyGenData<L>) -> syn::ItemMod {
    ...
    let ccf_impls = match tgd.id {
        ...
        Some(AlgebraicSortId::Sum(_)) => ccf_tys
            .iter()
            .zip(ccf_tys_flattened.iter())
            .zip(ccf_sort_camel_idents.iter())
            .map(|((ccfty, ccftyflat), ccf_camel)| {
                gen_ccf_impl_sum(bps, camel, ccfty, ccftyflat, ccf_camel)
            })
            .collect(),
    };
    syn::parse_quote! {
