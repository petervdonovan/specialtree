...
>(input: &str) -> Result<(LImpl, Nat), ()> {
    let refbased: crate::data_structure::refbased::Nat = serde_json::from_str(input)
...
fn unparse<
    LImpl: crate::extension_of::LImpl,
    Nat: crate::extension_of::owned::Nat<LImpl = LImpl>,
>(lo: &LImpl, own: Nat) -> Result<String, ()> {
    use crate::extension_of::reference::Nat;
    let mut l = core::default::Default::default();
    let refbased: crate::data_structure::refbased::Nat = own
        .get_ref(lo)
        .convert(lo, &mut l);
...
fn round_trip() {
    ...
    let (lo, nat) = parse::<
        crate::data_structure::idxbased::Fib,
        crate::data_structure::idxbased::Nat,
    >(&input)
        .unwrap();
